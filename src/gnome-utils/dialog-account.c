...
gnc_edit_account_ok(AccountWindow *aw)
{
  ...
  if (!gnc_account_is_root(new_parent))
  {
    int parent_type;
  ...
  if (change_all)
  {
    ...
    root = gnc_account_get_root(new_parent);
    do
    {
      ancestor = temp;
      temp = gnc_account_get_parent(ancestor);
    } while (temp != root);
                               ...
                               GList *valid_types )
{
  ...
  aw = gnc_ui_new_account_window_internal( book, NULL, NULL, validTypesCopy, NULL, FALSE );
