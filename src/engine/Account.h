...
gnc_account_get_book (const Account *acc)
{
  return qof_instance_get_book(QOF_INSTANCE(acc));
}
...
/** @name Getting Accounts and Subaccounts by Name
 @{
*/
/** The gnc_account_lookup_name() subroutine fetches the account by
 ...
 */
Account *gnc_account_lookup_name (const Account *parent, const char *name);

/** The gnc_account_lookup_full_name() subroutine works like
 *  gnc_account_lookup_name, but uses fully-qualified names using the
 *  given separator.
 */
Account *gnc_account_lookup_full_name (const Account *any_account,
				       const gchar *name);
