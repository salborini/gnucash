...
GList *
gnc_account_get_children (const Account *acc)
{
  return acc ? g_list_copy(acc->children) : NULL;
}
...
Account *
gnc_account_lookup_name (const Account *parent, const char * name)
{
  ...
  {
    child = node->data;
    result = gnc_account_lookup_name (child, name);
    if (result)
      return result;
  }
...
static Account *
gnc_account_lookup_full_name_helper (const Account *parent,
				     gchar **names)
{
  Account *found;
  GList *node;
  ...
  for (node = parent->children; node; node = node->next) {
	...
	return NULL;
      ...
      found = gnc_account_lookup_full_name_helper(account, &names[1]);
      if (found != NULL) {
	return found;
      }
    }
...
Account *
gnc_account_lookup_full_name (const Account *any_acc,
			      const gchar *name)
{
  const Account *root;
  Account *found;
  ...
  found = gnc_account_lookup_full_name_helper(root, names);
  g_strfreev(names);
  return found;
}
