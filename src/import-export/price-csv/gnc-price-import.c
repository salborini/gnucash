...
\********************************************************************/
/** @file gnc-price-import.c
    @brief Price Import GUI code
...
typedef struct
{
    GncPriceParseData* parse_data;     /**< The actual data we are previewing */
    GtkDialog* dialog;
    ...
    GtkCheckButton* skipheader_cbutton;/**< The checkbutton to skip header line or not */
    ...
    gboolean not_empty;                /**< FALSE initially, true after the first type 
					* gnc_price_preview_update is called. */
...
static void gnc_price_preview_update(GncPricePreview* preview);
 ...
 */
static void sep_button_clicked(GtkWidget* widget, GncPricePreview* preview)
{
    int i;
    ...
    if (gnc_price_parse(preview->parse_data, FALSE, NULL))
    {
    ...
    gnc_price_preview_update(preview);
}
 ...
 */
static void separated_or_fixed_selected(GtkToggleButton* csv_button, GncPricePreview* preview)
{
    GError* error = NULL;
    ...
    /* Reparse the data. */
    if (gnc_price_parse(preview->parse_data, FALSE, &error))
    {
...
/** Event handler for toggling the checkbox to skip header line.
 * @param widget The widget that was changed
 * @param preview The data that is being configured
 */
static void skipheader_checkbox_clicked(GtkWidget* widget, GncPricePreview* preview)
{
    GError* error = NULL;
    gboolean skip = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(preview->skipheader_cbutton));
    preview->parse_data->skip_headerline = skip;
			      ...
			      GncPricePreview* preview)
{
	...
	if (gnc_price_convert_encoding(preview->parse_data, encoding, &error) ||
	    gnc_price_parse(preview->parse_data, FALSE, &error))
	{
 ...
 * @param preview The display of the data being imported
 */
static void date_format_selected(GtkComboBox* format_selector, GncPricePreview* preview)
{
 ...
 * @param preview The display of the data being imported
 */
static void ok_button_clicked(GtkWidget* widget, GncPricePreview* preview)
{
	...
	for (type = 0; type < GNC_PRICE_NUM_COL_TYPES; type++)
	{
	    /* ... we find one that matches with what's in the column. */
	    if (!g_strcmp0(contents, _(gnc_price_column_type_strs[type])))
	    {
    ...
    preview->approved = TRUE; /* The user wants to do the import. */
}
 ...
 * @param preview The display of the data being imported
 */
static void cancel_button_clicked(GtkWidget* widget, GncPricePreview* preview)
{
    gtk_widget_hide((GtkWidget*)(preview->dialog));
}
 ...
 * @param preview The display of the data being imported
 */
static void treeview_resized(GtkWidget* widget, GtkAllocation* allocation, GncPricePreview* preview)
{
...
static void column_type_changed(GtkCellRenderer* renderer, gchar* path,
				GtkTreeIter* new_text_iter, GncPricePreview* preview)
{
    /* ncols is the number of columns in the data. */
    int i, ncols = preview->parse_data->column_types->len;
	...
	if (col_renderer != renderer)
	{
		...
		gtk_list_store_set(GTK_LIST_STORE(store), &iter, 2 * i + 1,
				   _(gnc_price_column_type_strs[GNC_PRICE_NONE]), -1);
	    }
	    g_free(contents);
	}
...
/** Constructor for GncPricePreview.
 * @return A new GncPricePreview* ready for use.
 */
static GncPricePreview* gnc_price_preview_new()
{
    int i;
    GncPricePreview* preview = g_new(GncPricePreview, 1);
    ...
    gnc_builder_add_from_file (builder, "gnc-price-import.glade", "preview-dialog");
    ...
    /* Load and connect the skipheader checkbutton. */
    preview->skipheader_cbutton =
	(GtkCheckButton*)GTK_WIDGET(gtk_builder_get_object (builder, "skipheader_cbutton"));
    g_signal_connect(G_OBJECT(preview->skipheader_cbutton), "clicked",
		     G_CALLBACK(skipheader_checkbox_clicked), (gpointer)preview);
...
/** Destructor for GncPricePreview. This does not free
 * preview->parse_data, which must be freed separately.
 * @param preview The preview whose memory is freed.
 */
static void gnc_price_preview_free(GncPricePreview* preview)
{
    g_free(preview);
}
 ...
 */
static GtkCellRenderer* gnc_price_preview_get_cell_renderer(GncPricePreview* preview, int col)
{
...
static gboolean
make_new_column (GncPricePreview *preview, int col, int dx, gboolean test_only)
{
    PangoLayout *layout;
    PangoFontDescription *font_desc;
    int charindex, width;
    GtkCellRenderer *cell =	gnc_price_preview_get_cell_renderer(preview, col);
    int colstart, colend;
    GError* error = NULL;
...
static gboolean
widen_column (GncPricePreview *preview, int col, gboolean test_only)
{
...
static gboolean
narrow_column (GncPricePreview *preview, int col, gboolean test_only)
{
    int colcount = stf_parse_options_fixed_splitpositions_count (preview->parse_data->options);
...
static gboolean
delete_column (GncPricePreview *preview, int col, gboolean test_only)
{
    GError* error = NULL;
    int colcount = stf_parse_options_fixed_splitpositions_count (preview->parse_data->options);
...
static void
select_column (GncPricePreview *preview, int col)
{
    GError* error = NULL;
    int colcount = stf_parse_options_fixed_splitpositions_count (preview->parse_data->options);
...
fixed_context_menu_handler (GnumericPopupMenuElement const *element,
			    gpointer user_data)
{
    GncPricePreview *preview = user_data;
...
static void
fixed_context_menu (GncPricePreview *preview, GdkEventButton *event,
		    int col, int dx)
{
...
static void gnc_price_preview_update(GncPricePreview* preview)
{
	...
	for (j = 0; j < GNC_PRICE_NUM_COL_TYPES; j++)
	{
	    ...
	    gtk_list_store_set(cstores[i], &iter, 0, _(gnc_price_column_type_strs[j]), -1);
	}
    }
	...
	gtk_list_store_set(ctstore, &iter, 2 * i, cstores[i], 2 * i + 1,
			   _(gnc_price_column_type_strs[(int)(preview->parse_data->column_types->data[i])]),
			   -1);
    }
 ...
 */
static int gnc_price_preview(GncPricePreview* preview, GncPriceParseData* parse_data)
{
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(preview->skipheader_cbutton),
				 parse_data->skip_headerline);

    /* Load the data into the treeview. (This is the first time we've
     * called gnc_price_preview_update on this preview, so we use
     * FALSE.) */
    gnc_price_preview_update(preview);
 ...
 * function must only be called after calling gnc_price_preview. It is
 * essentially identical in behavior to gnc_price_preview except that it
 ...
 */
static int gnc_price_preview_errors(GncPricePreview* preview)
{
    gchar* name;
    GtkIconSize size;
...
static void display_report(GncPriceImportReport *report)
{
    if (!report) return;
	...
	if (report->nb_skipped == 0)
	    gnc_info_dialog(NULL, "Nothing to import.");
	else
	    gnc_info_dialog(NULL, "Nothing imported.\nAll %d prices are already in your book.",
			    report->nb_skipped);
    }
    else
    {
	char *imported_part;
	char *skipped_part;
	char *securities_part;

	imported_part = g_strdup_printf("\n- %d prices imported",
					report->nb_imported);
	if (report->nb_skipped == 0)
	    skipped_part = g_strdup("");
	else
	    skipped_part = g_strdup_printf("\n- %d duplicate price(s) have been ignored",
					   report->nb_skipped);
	if (report->nb_securities_created == 0)
	    securities_part = g_strdup("");
	else
	    securities_part = g_strdup_printf("\n- %d securities imported - edit them in the Security Editor",
					      report->nb_securities_created);

	gnc_info_dialog(NULL, "Import complete.%s%s%s", imported_part, skipped_part, securities_part);
	g_free(imported_part);
	g_free(skipped_part);
	g_free(securities_part);
    }
}
...
void gnc_file_price_import(void)
{
    ...
    gtk_file_filter_set_name(filter, _("CSV/Fixed width (*.csv *.tsv *.txt)"));
    gtk_file_filter_add_pattern(filter, "*.[Cc][Ss][Vv]");
    gtk_file_filter_add_pattern(filter, "*.[Tt][Ss][Vv]");
    gtk_file_filter_add_pattern(filter, "*.[Tt][Xx][Tt]");
    filters = g_list_append(filters, filter);

    /* Let the user select a file. */
    filename = gnc_file_dialog(_("Select an CSV/Fixed-Width file to import"),
	...
	GError* error = NULL;
	GList* prices; /* A list of the prices we create */
	GncPriceParseData* parse_data;
	GncPricePreview* preview;
	GncPriceImportReport report;
	...
	parse_data = gnc_price_new_parse_data();
	if (gnc_price_load_file(parse_data, filename, &error))
	{
	    /* If we couldn't load the file ... */
	    gnc_error_dialog(NULL, "%s", error->message);
	    if (error->code == GNC_PRICE_FILE_OPEN_ERR)
	    {
		gnc_price_parse_data_free(parse_data);
		g_free(filename);
		return;
	    }
	...
	/* Parse the data. */
	if (gnc_price_parse(parse_data, TRUE, &error))
	{
	...
	/* Preview the data. */
	preview = gnc_price_preview_new();
	if (gnc_price_preview(preview, parse_data))
	{
	    /* If the user clicked "Cancel", free everything and quit. */
	    gnc_price_preview_free(preview);
	    gnc_price_parse_data_free(parse_data);
	    g_free(filename);
	    return;
	}
	...
	gnc_price_parse_to_prices(parse_data, FALSE);
	...
	{
	    user_canceled = gnc_price_preview_errors(preview);
	    gnc_price_parse_to_prices(parse_data, TRUE);
	}

	/* Do the actual import to the book's pricedb */
	gnc_price_do_import(parse_data->prices, &report);
